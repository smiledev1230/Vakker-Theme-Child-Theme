...
/*
   Class: VakkerElatedTaxonomyField
   A class that initializes VakkerElated Taxonomy Field
*/

class VakkerElatedTaxonomyField implements iVakkerElatedRender {
	private $type;
	private $name;
	private $label;
	private $description;
	...
	function __construct( $type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden_property = "", $hidden_values = array() ) {
		...
		add_filter( 'vakker_eltd_taxonomy_fields', array( $this, 'addFieldForEditSave' ) );
	}
...
abstract class VakkerElatedTaxonomyFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array() );
}

class VakkerElatedTaxonomyFieldText extends VakkerElatedTaxonomyFieldType {
...
class VakkerElatedTaxonomyFieldImage extends VakkerElatedTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false) {
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			<div class="form-field">
				<label for="<?php echo esc_html( $name ); ?>"><?php echo esc_html( $label ); ?></label>
				...
				<p>
					<input type="button" class="button button-secondary eltd-tax-media-add" name="eltd-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'vakker' ); ?>"/>
					<input type="button" class="button button-secondary eltd-tax-media-remove" name="eltd-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'vakker' ); ?>"/>
				</p>
			</div>
			<?php
		} else {
			...
			<tr class="form-field" <?php if ($hidden) { ?> style="display: none"<?php } ?>>
				...
				<td>
					...
					<p>
						<input type="button" class="button button-secondary eltd-tax-media-add" name="eltd-tax-media-add" value="<?php esc_attr_e( 'Add Image', 'vakker' ); ?>"/>
						<input data-termid="<?php echo esc_html( $_GET['tag_ID'] ); ?>" data-taxonomy="<?php echo esc_html( $taxonomy ); ?>" type="button" class="button button-secondary eltd-tax-media-remove" name="eltd-tax-media-remove" value="<?php esc_attr_e( 'Remove Image', 'vakker' ); ?>"/>
					</p>
				</td>
			</tr>
			<?php
...
class VakkerElatedTaxonomyFieldSelect extends VakkerElatedTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class VakkerElatedTaxonomyFieldIcon extends VakkerElatedTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		$options           = \VakkerElatedIconCollections::get_instance()->getIconCollectionsEmpty();
		$icons_collections = \VakkerElatedIconCollections::get_instance()->getIconCollectionsKeys();
		...
		if ( ! isset( $_GET['tag_ID'] ) ) { ?>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				<div class="form-field eltd-icon-collection-holder" style="display: none" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
					<label for="<?php echo esc_attr( $name ) . '_icon'; ?>"><?php esc_html_e( 'Icon', 'vakker' ); ?></label>
			...
			<?php foreach ( $icons_collections as $icons_collection ) {
				...
				<tr class="form-field eltd-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
					<th scope="row"><?php esc_html_e( 'Icon', 'vakker' ); ?></th>
					<td>
						<select name="<?php echo esc_attr( $name . '_' . $icons_param ) ?>" id="<?php echo esc_attr( $name . '_' . $icons_param ) ?>">
							<?php
							$icons      = \VakkerElatedIconCollections::get_instance()->getIconCollection( $icons_collection );
...
class VakkerElatedTaxonomyFieldColor extends VakkerElatedTaxonomyFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
...
class VakkerElatedTaxonomyFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $hidden = false ) {
		
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new VakkerElatedTaxonomyFieldText();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'image':
				$field = new VakkerElatedTaxonomyFieldImage();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'selectblank':
				$field = new VakkerElatedTaxonomyFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'icon':
				$field = new VakkerElatedTaxonomyFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
			
			case 'color':
				$field = new VakkerElatedTaxonomyFieldColor();
				$field->render( $name, $label, $description, $options, $args, $hidden );
				break;
