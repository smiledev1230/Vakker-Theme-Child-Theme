...
namespace VakkerElated\Modules\Header\Lib;
/**
 * Class VakkerElatedHeaderConnector
 *
...
class VakkerElatedHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'vakker_eltd_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'vakker_eltd_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'vakker_eltd_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'vakker_eltd_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to vakker_eltd_title_content_padding filter
	 *
	 * @return int
	 */
