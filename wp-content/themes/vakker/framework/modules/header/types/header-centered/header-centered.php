...
class HeaderCentered extends HeaderType {
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			$this->logoAreaHeight     = vakker_eltd_set_default_logo_height_for_header_types();
	...
	public function loadTemplate( $parameters = array() ) {
		...
		$parameters['logo_area_in_grid'] = vakker_eltd_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' ? true : false;
		$parameters['menu_area_in_grid'] = vakker_eltd_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' ? true : false;
		
		$parameters = apply_filters( 'vakker_eltd_header_centered_parameters', $parameters );
	...
	public function calculateHeightOfTransparency() {
		...
		$logo_background_color             = vakker_eltd_get_meta_field_intersect( 'logo_area_background_color', $id );
		$logo_background_transparency      = vakker_eltd_get_meta_field_intersect( 'logo_area_background_transparency', $id );
		$logo_grid_background_color        = vakker_eltd_options()->getOptionValue( 'logo_area_grid_background_color' );
		$logo_grid_background_transparency = vakker_eltd_options()->getOptionValue( 'logo_area_grid_background_transparency' );
		...
		$menu_background_color             = vakker_eltd_get_meta_field_intersect( 'menu_area_background_color', $id );
		$menu_background_transparency      = vakker_eltd_get_meta_field_intersect( 'menu_area_background_transparency', $id );
		...
		if ( $logoAreaTransparent || $menuAreaTransparent ) {
			if ( $logoAreaTransparent ) {
				...
				if ( ( $sliderExists && vakker_eltd_is_top_bar_enabled() )
				     || vakker_eltd_is_top_bar_enabled() && vakker_eltd_is_top_bar_transparent()
				) {
					$transparencyHeight += vakker_eltd_get_top_bar_height();
				}
			}
	...
	public function calculateHeightOfCompleteTransparency() {
		...
		$logo_background_color             = vakker_eltd_options()->getOptionValue( 'logo_area_background_color' );
		$logo_background_transparency      = vakker_eltd_options()->getOptionValue( 'logo_area_background_transparency' );
		$logo_grid_background_color        = vakker_eltd_options()->getOptionValue( 'logo_area_grid_background_color' );
		$logo_grid_background_transparency = vakker_eltd_options()->getOptionValue( 'logo_area_grid_background_transparency' );
		...
		if ( $logoAreaTransparent || $menuAreaTransparent ) {
			if ( $logoAreaTransparent ) {
				...
				if ( vakker_eltd_is_top_bar_enabled() && vakker_eltd_is_top_bar_completely_transparent() ) {
					$transparencyHeight += vakker_eltd_get_top_bar_height();
				}
			}
