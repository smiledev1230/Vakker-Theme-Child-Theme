...
class VakkerElatedLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_vakker_eltd_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_vakker_eltd_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'vakker' );
		
		if ( isset( $_COOKIE[ 'eltd-like_' . $post->ID ] ) ) {
			$class = 'eltd-like liked';
			$title = esc_html__( 'You already like this!', 'vakker' );
		}
...
if ( ! function_exists( 'vakker_eltd_create_like' ) ) {
	function vakker_eltd_create_like() {
		VakkerElatedLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'vakker_eltd_create_like' );
}